...
exports.signup = async (req, res) => {
    ...
    var resUser = await User.create({
        ...
        .then(async user => {
            ...
            var resSwabber = await createUserSwab(user, req);
            console.log(resSwabber);
            if (resDetail.status && resSwabber.status) {
                responseTemp = {
                    status: true,
...
async function createUserSwab(user, req) {
    var ipInfo = getIP(req);
    var response = new Array();
    var responseTemp = new Array();

    var resUserSwab = await UserSwab.create({
        clientId: 1,
        iduser: user.id,
        created_by: JSON.stringify(ipInfo),
    })
...
exports.signin = async (req, res) => {
    ...
    try {
        ...
        var resUser = await User.findOne({
            where: { username: req.body.username, level: 2 },
            include: [
                {
                    model: UserDetail,
                    required: true
            ...
            .then(user => {
                if (!user) {
                    ...
                    if (!passwordIsValid) {
                        ...
                        var token = jwt.sign({ id: user.id }, config.secretSwabber, {
                            expiresIn: '365d'
                        });
                        responseTemp = {
                            status: true,
